...
class Allocator(ABC):
    ...
    def allocate(self, global_thrust, relax=True, current_state=None):
        ...
        for combination in itertools.product(*disjuncts):
            ...
            C, b, n_eq = self.assemble_constraints(global_thrust, relax, combination, current_state!=None)

            try:
                res = quadprog.solve_qp(G, a, C, b, n_eq)
                results[res[1]] = res
            except ValueError:
